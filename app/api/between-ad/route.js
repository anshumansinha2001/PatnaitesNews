...
export async function POST(request) {
  try {
    ...
    // Check the count of existing ads
    const count = await BetweenAdsModel.countDocuments();

    // If there are already two ads, prevent insertion
    if (count >= 2) {
      return NextResponse.json({
        success: false,
        message: "Only two advertisements can be stored.",
      });
    }
    ...
    await BetweenAdsModel.create(adData);
...
export async function PUT(request) {
  ...
  try {
    ...
    await BetweenAdsModel.findByIdAndUpdate(id, updatedAdData, { new: true });
    return NextResponse.json({
      success: true,
...
export async function GET(request) {
  ...
  try {
    ...
    if (id) {
      ...
      const ads = await BetweenAdsModel.find().sort({ createdAt: -1 });
...
export async function DELETE(request) {
  ...
  try {
    ...
    await BetweenAdsModel.findByIdAndDelete(id);
